...
def main():
    ...
    # Widgets
    windex = col1.multiselect("SPI Index", list(set(df1.Index)), [])
    if not windex:
        windex = list(set(df1.Index))

    month = col1.multiselect("Month of forecast issue", list(set(df1.Month_of_issue)), [])
    if not month:
        month = list(set(df1.Month_of_issue)) 



    district = col2.multiselect("District", list(set(df1.District)), [])
    if not district:
        district = list(set(df1.District))

    category = col2.multiselect("Category", list(set(df1.Category)), [])
    if not category:
